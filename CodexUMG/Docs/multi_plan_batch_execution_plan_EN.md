...
## Document Purpose
- This document organizes the dependency graph, start conditions, and parallel execution boundaries of the four plans below into a single top-level execution plan.
...
- The main worker first analyzes all four plans, then finalizes shared edit points and responsibility boundaries.
...
1. Read all four plan documents and identify shared dependencies, potential conflict files, detailed execution procedures inside sub-documents, and blocking conditions.
...
1. Verify the integrated results of Track C and Track D on top of the foundation from Track A.
2. Verify Track B's VOX results separately, then merge them at the main-branch level.
