...
- 좌클릭으로 기본 무기를 발사하고, 적 명중 판정은 라인트레이스 채널로 처리한다.
...
- `DA_*GlobalFxConfig`(또는 동등한 전역 피드백/FX DA)는 실제 프로젝트 애셋으로 존재해야 하며, `GameInstance`가 이를 참조해 전역 접근 지점을 제공해야 한다.
- 피격 피드백에 필요한 `Niagara System`과 카메라 흔들림 레퍼런스는 전역 피드백/FX DA에 집계한다.
...
- 플레이어 공격의 적 명중 판정은 라인트레이스 채널을 사용하고, 적은 해당 공격 판정 채널에 응답해야 한다.
...
- 런타임 피드백 연결에 필요한 `DA_*GlobalFxConfig`, 피격 `Niagara System`, 카메라 흔들림 애셋도 커맨드렛으로 생성, 갱신, 연결 가능해야 한다.
...
- 전역 피드백/FX 접근도 헤드리스 경로에서 `GameInstance` 또는 동등한 전역 접근 레이어가 `DA_*GlobalFxConfig`를 참조하도록 완료되어야 한다.
- 라인트레이스 채널 기반 공격 판정에 필요한 Trace Channel 설정과 응답 규칙은 코드, 설정, 문서에 함께 남긴다.
...
- 게임 모드, 게임 상태 흐름
- 플레이어 컨트롤러, 플레이어 캐릭터
- 이동, 조준, 발사, 라인트레이스 공격 판정, 데미지, 체력, 피격 반동, 사망
- 적 추적 및 공격 로직
- `GameInstance` 기반 전역 피드백/FX 레지스트리 접근
- 웨이브 스폰과 진행 규칙
- UI에 제공할 데이터 구조와 이벤트
- 커맨드렛 및 에디터 자동화 코드
...
- `DA_*GlobalFxConfig`, `Niagara System`, 카메라 흔들림 애셋 생성 및 참조 구성
- 메쉬, 머터리얼, 이펙트, 사운드 연결
- 입력 액션과 매핑 컨텍스트 에셋
- 적 명중용 충돌과 Trace Channel 응답 설정
- 위젯 레이아웃과 표현
- 밸런스 수치 조정
- 스폰 포인트, 레벨 배치
...
- 같은 파일을 동시에 수정해야 하는 구조는 기본 경로로 사용하지 않는다.
...
- 기본 발사 입력
- 적 명중용 라인트레이스 채널 설정 및 플레이어 공격 판정
- `DA_*GlobalFxConfig` 생성 및 `GameInstance` 전역 접근 연결
- 체력 컴포넌트
- 피격과 사망 처리
