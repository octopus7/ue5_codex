...
`DA_*GlobalFxConfig` 같은 전역 피드백 DA를 두고, `GameInstance`가 그 DA를 참조해서 NS와 카메라 흔들림에 전역 접근 가능하게 해.
플레이어 공격은 적 명중용 라인트레이스 채널을 사용하고, 플레이어 피격시에는 반동, 피격 NS, 카메라 흔들림이 같이 동작하게 해.
```
...
- 전역 피드백 구조를 강하게 통제하고 싶으면 `DA_*GlobalFxConfig`, `GameInstance 전역 접근`, `피격 NS`, `카메라 흔들림`, `피격 반동`을 함께 적는 편이 가장 명확하다.
- 공격 판정 방식을 고정하고 싶으면 `적 명중용 라인트레이스 채널`을 함께 적는 편이 가장 명확하다.
