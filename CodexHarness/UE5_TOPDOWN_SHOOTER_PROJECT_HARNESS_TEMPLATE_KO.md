...
- 캐릭터 외형 규칙: 캐릭터 Blueprint 내부 메시 컴포넌트 또는 동등한 렌더링 컴포넌트가 프로젝트 내 생성된 메시 애셋을 실제 참조하게 할 것
- 입력 애셋 규칙: `IA_*`, `IMC_*`, `DA_*InputConfig`를 실제 프로젝트 애셋으로 만들 것
- 입력 연결 규칙: 플레이어는 `IA_*`나 `IMC_*`를 직접 참조하지 말고, 이를 모은 `DA_*InputConfig` 애셋만 참조할 것
...
- `BP_*Character` 내부 메시 컴포넌트에 import 된 프로젝트 메시 애셋을 실제 할당해줘.
- `IA_*`, `IMC_*`, `DA_*InputConfig`를 커맨드렛으로 실제 생성 또는 갱신해줘.
- 플레이어에는 `DA_*InputConfig`만 연결하고 `IA_*`/`IMC_*` 직접 참조는 남기지 말아줘.
...
- 변경 후 아래 항목을 보고해줘:
  ...
  - 캐릭터 Blueprint 내부 메시 컴포넌트에 실제로 할당된 메시 애셋 경로
  - 실제 생성된 `IA_*`, `IMC_*`, `DA_*InputConfig` 애셋 경로와 플레이어에 연결된 DA 경로
```
